...
func main() {
	...
	TEXT("KeccakF1600x4", NOSPLIT, "func(a *[100]uint64)")

	Pragma("noescape")

	statePtr := Load(Param("a"), GP64())
	state := func(offset int) Op {
		return Mem{Base: statePtr, Disp: 32 * offset}
	}

	rcData := GLOBL("rc", RODATA|NOPTR)
	...
	superRound := GP64()
	MOVQ(U64(6), superRound) // 6 super-rounds * 4 = 24 rounds
