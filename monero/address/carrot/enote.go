...
func makeEnoteEphemeralPrivateKey(anchor, inputContext []byte, spendPub crypto.PublicKeyBytes, paymentId [8]byte) *crypto.PrivateKeyScalar {
	// k_e = (H_64(anchor_norm, input_context, K^j_s, pid)) mod l
	transcript := FixedTranscript([]byte(DomainSeparatorEphemeralPrivateKey), anchor, inputContext, spendPub[:], paymentId[:])
...
func makeEnoteEphemeralPublicKeySubaddress(key crypto.PrivateKey, spendKey crypto.PublicKey) (out crypto.X25519PublicKey) {
	// K_e = d_e K^j_s
	D_e_in_ed25519 := key.AsScalar().GetDerivation(spendKey.AsPoint()).AsPoint().Point()

	// D_e = ConvertPointE(K_e)
	return crypto.ConvertPointE(D_e_in_ed25519)
}
