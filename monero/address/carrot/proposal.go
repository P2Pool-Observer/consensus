...
func (p *PaymentProposalV1) EnoteEphemeralPrivateKey(inputContext []byte) crypto.PrivateKey {
	...
	return makeEnoteEphemeralPrivateKey(p.Randomness[:], inputContext, p.Destination.Address[address.PackedAddressSpend], p.Destination.PaymentId)
}

func (p *PaymentProposalV1) EnoteEphemeralPublicKey(key crypto.PrivateKey) (out crypto.X25519PublicKey) {
	if p.Destination.IsSubaddress {
		return makeEnoteEphemeralPublicKeySubaddress(key, &p.Destination.Address[address.PackedAddressSpend])
	} else {
		return makeEnoteEphemeralPublicKeyCryptonote(key)
	}
}

func (p *PaymentProposalV1) Output(blockIndex uint64) (*CoinbaseEnoteV1, error) {
	...
	inputContext := make([]byte, 0, 1+types.HashSize)
	inputContext = append(inputContext, DomainSeparatorInputContextCoinbase)
	inputContext = binary.LittleEndian.AppendUint64(inputContext, blockIndex)
