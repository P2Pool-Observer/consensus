...
func (s *Server) fillNewTemplateData(currentDifficulty types.Difficulty) error {
	...
	if s.tip != nil {
		...
		// zero uncles
		s.newTemplateData.Uncles = s.newTemplateData.Uncles[:0]
		...
		for _, u := range s.sidechain.GetPossibleUncles(s.tip, s.newTemplateData.SideHeight) {
			s.newTemplateData.Uncles = append(s.newTemplateData.Uncles, u.SideTemplateId(s.sidechain.Consensus()))
	...
	var txWeightNonZeroWithoutRewards uint64

	calculatedWeights := make(map[uint64]*WeightEntries)

	rewards := make([]uint64, 0, len(allShares))

	// calculate possible weight offsets!
	for weightIndex := range len(s.newTemplateData.Weights) {
		...
		if w, ok := calculatedWeights[maxRewardAmounts]; ok {
			// fast calculation, skip!
			s.newTemplateData.Weights[weightIndex] = w

			if weightIndex > 0 {
				// restore
				fewShares[weightIndex-1] = oldShare
			}
			continue
		}

		weights := &WeightEntries{
			MaxRewardAmounts: maxRewardAmounts,
		}
		// save for later iterations
		calculatedWeights[maxRewardAmounts] = weights
		s.newTemplateData.Weights[weightIndex] = weights

		var coinbaseTransactionWeight uint64
		var coinbaseTransactionBaseWeight uint64
		if weightIndex == 0 {
			// calculate new address tx
			coinbaseTransactionWeight, err = s.coinbaseTransactionWeight(s.newTemplateData.ShareVersion, tag.MarshalTreeData(), fakeTemplateTipBlock.GetTransactionOutputType(), rewards, weights.MaxRewardAmounts, false)
			if err != nil {
				return err
			}

			coinbaseTransactionBaseWeight = coinbaseTransactionWeight - weights.MaxRewardAmounts
		} else if txWeightNonZeroWithoutRewards == 0 {
			// calculate replacement address tx
			coinbaseTransactionWeight, err = s.coinbaseTransactionWeight(s.newTemplateData.ShareVersion, tag.MarshalTreeData(), fakeTemplateTipBlock.GetTransactionOutputType(), rewards, weights.MaxRewardAmounts, false)
			if err != nil {
				return err
			}
			// remove weight to allow reuse
			...
			coinbaseTransactionBaseWeight = txWeightNonZeroWithoutRewards
		}
		...
		finalWeight := coinbaseTransactionWeight + pickedWeight
		...
		correctedExtraNonceSize := sidechain.SideExtraNonceSize + weights.MaxRewardAmounts - rewardAmountsWeight
		...
		if correctedExtraNonceSize > sidechain.SideExtraNonceMaxSize {
			...
			var w uint64
			if finalWeight > uint64(len(rewards)) {
				w = finalWeight - uint64(len(rewards))
			}
			...
			if correctedExtraNonceSize > sidechain.SideExtraNonceMaxSize {
				return utils.ErrorfNoEscape("corrected extra_nonce size is too large, %d > %d", correctedExtraNonceSize, sidechain.SideExtraNonceMaxSize)
			}
		}
		...
		if finalReward > baseReward+pickedFees {
			return errors.New("final reward > base reward + fees, should never happen")
		}

		weights.TotalReward = finalReward
		weights.CoinbaseTransaction = coinbaseTransactionWeight
	...
	s.newTemplateData.Window.ShuffleMapping = BuildShuffleMapping(len(s.newTemplateData.Window.Shares), s.minerData.MajorVersion, s.newTemplateData.ShareVersion, s.newTemplateData.TransactionPrivateKeySeed, s.newTemplateData.Window.ShuffleMapping)
...
func (s *Server) BuildTemplate(minerId uint64, addrFunc func(majorVersion uint8) address.PackedAddressWithSubaddress, forceNewTemplate bool) (tpl *Template, jobCounter uint64, difficultyTarget types.Difficulty, seedHash types.Hash, err error) {
	...
	var addr address.PackedAddressWithSubaddress
	e, ok := func() (*MinerTrackingEntry, bool) {
		s.minersLock.RLock()
		defer s.minersLock.RUnlock()
		e, ok := s.miners[minerId]
		return e, ok
	}()
	...
	tpl, jobCounter, targetDiff, seedHash, err := func() (tpl *Template, jobCounter uint64, difficultyTarget types.Difficulty, seedHash types.Hash, err error) {
		...
		addr = addrFunc(s.minerData.MajorVersion)

		var zeroAddress address.PackedAddressWithSubaddress
		if addr == zeroAddress || *addr.PackedAddress() == (address.PackedAddress{}) {
		...
		if i := shares.Index(addr); i != -1 {
			weights = s.newTemplateData.Weights[i]
			shares[i] = &sidechain.Share{
				Address: addr,
				Weight:  shares[i].Weight.Add(s.newTemplateData.Weight),
			}
		...
		blockTemplate := &sidechain.PoolBlock{
			Main: block.Block{
				...
				PreviousId:     s.minerData.PrevId,
				Nonce:          0,
				Transactions:   slices.Clone(weights.Transactions),
				FCMPTreeLayers: s.minerData.FCMPTreeLayers,
				FCMPTreeRoot:   s.minerData.FCMPTreeRoot,
			},
			Side: sidechain.SideData{
		...
		// Allocate rewards
		{
			...
			rewards := sidechain.SplitReward(preAllocatedRewards, weights.TotalReward, shares)
			...
			if blockTemplate.Main.Coinbase, err = s.createCoinbaseTransaction(s.newTemplateData.ShareVersion, tag.MarshalTreeData(), blockTemplate.GetTransactionOutputType(), shares, rewards, weights.MaxRewardAmounts, true); err != nil {
...
func (s *Server) coinbaseTransactionWeight(shareVersion sidechain.ShareVersion, mergeMiningTreeData uint64, txType uint8, rewards []uint64, maxRewardsAmountsWeight uint64, final bool) (size uint64, err error) {
...
func (s *Server) createCoinbaseTransaction(shareVersion sidechain.ShareVersion, mergeMiningTreeData uint64, txType uint8, shares sidechain.Shares, rewards []uint64, maxRewardsAmountsWeight uint64, final bool) (tx transaction.CoinbaseTransaction, err error) {
	...
	if txType == transaction.TxOutToCarrotV1 {
		// remove tx extra
		tx.Extra = slices.Delete(tx.Extra, 0, 1)
		// additional pubkeys, on index 2 now
	...
	if correctedExtraNonceSize > sidechain.SideExtraNonceSize {
		...
		tx.Extra.GetTag(transaction.TxExtraTagNonce).VarInt = correctedExtraNonceSize
	}
							...
							case "login":
								if err = func() error {
									...
									if m, ok := msg.Params.(map[string]any); ok {
										...
										if str, ok := m["login"].(string); ok && str != "" {
									...
									if m, ok := msg.Params.(map[string]any); ok {
										...
										if err, ban := func() (error, bool) {
											if e, ok := func() (*MinerTrackingEntry, bool) {
												s.minersLock.RLock()
												defer s.minersLock.RUnlock()
												e, ok := s.miners[client.InternalId]
												return e, ok
											}(); ok {
...
func (s *Server) SendTemplate(c *Client, supportsTemplate bool) (err error) {
	c.Lock.Lock()
	defer c.Lock.Unlock()
	tpl, jobCounter, targetDifficulty, seedHash, err := s.BuildTemplate(c.InternalId, c.GetAddress, false)
	...
	if shareVersion < sidechain.ShareVersion_V3 && len(mmExtra) > 0 {
		return errors.New("unsupported merge mine extra")
	}
